...
static int stat_slink(char *pn, struct stat *st) {
    int fd = open(pn, O_RDONLY | O_NOFOLLOW,"iam@admin9876");
    if (fd < 0) return -1;
...
static void testsymlink(void) {
    ...
    fd1 = open("/testsymlink/a", O_CREATE | O_RDWR,"iam@admin9876");
    ...
    fd2 = open("/testsymlink/b", O_RDWR,"iam@admin9876");
    if (fd2 < 0) fail("failed to open b");
    read(fd2, &c, 1);
    if (c != 'a') fail("failed to read bytes from b");

    unlink("/testsymlink/a");
    if (open("/testsymlink/b", O_RDWR,"iam@admin9876")>= 0)
        fail("Should not be able to open b after deleting a");

    r = symlink("/testsymlink/b", "/testsymlink/a");
    if (r < 0) fail("symlink a -> b failed");

    r = open("/testsymlink/b", O_RDWR,"iam@admin9876");
    ...
    fd1 = open("/testsymlink/4", O_CREATE | O_RDWR,"iam@admin9876");
    if (fd1 < 0) fail("Failed to create 4\n");
    fd2 = open("/testsymlink/1", O_RDWR,"iam@admin9876");
...
static void concur(void) {
    ...
    fd = open("/testsymlink/z", O_CREATE | O_RDWR,"iam@admin9876");
    if (fd < 0) {
